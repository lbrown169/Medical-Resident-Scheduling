...
public class Graph
{
    ...
    {
        level
            = new int[nodesAmt]; // array of size n where n is number of nodes
        ...
        {
            adjList.Add(
                new ArrayList()); // creating a row for every node in adj list but will be jagged, not every row is same size
        }
    }
    ...
    public void addEdge(int start, int end, int capacity) //adds to edgelist
    {
        (int start, int end, int capacity)
            edge = (start, end, capacity); //tuple in c#
        edgeList.Add(edge);
    }

    public void addEdgeToAdj(int start, int end, int capacity)
    {
        ((ArrayList)adjList[start]).Add(new Edge(end,
            ((ArrayList)adjList[end]).Count, capacity));
        ((ArrayList)adjList[end]).Add(new Edge(start,
            ((ArrayList)adjList[start]).Count - 1, 0));
    }

    private int dfs(int currNode, int sink, int flow)
    {
        if (currNode == sink ||
            flow == 0) // did we reach the end or did we run out of flow
        {
            return flow;
        }

        for ( /*start at prev index*/;
             pointers[currNode] <
             ((ArrayList)adjList[currNode]).Count;
             pointers[currNode]++)
        {
            Edge? currEdge =
                (Edge)((ArrayList)adjList[currNode])[
                    pointers[
                        currNode]]; //this is like a pointer.. but in c#

            if (level[currEdge.destination] ==
                level[currNode] +
                1) // if one level deeper, perform analysis
            {
                int push = dfs(currEdge.destination, sink,
                    Math.Min(flow, currEdge.currentCap));
                if (push != 0)
                {
                    currEdge.currentCap -= push;
                    ((Edge)((ArrayList)adjList[currEdge.destination])[
                        currEdge.reverse]).currentCap += push;
                    return push;
                }
            }
        }
    ...
    public int getFlow(int source, int sink)
    {
        ...
        for (int i = 0;
             i < edgeList.Count;
             i++) // take edges from edge list, permute them, then put them in the adjacency list
        {
            int j = rand.Next(i,
                edgeList
                    .Count); // generating a random index between the current location and end of list
        ...
        while (currFlow != 0)
        {
            ...
            while (qStart < qEnd && level[sink] == 0)
            {
                ...
                qStart++;
                for (int listIndex = 0;
                     listIndex < ((ArrayList)adjList[currNode]).Count;
                     listIndex++)
                {
                    Edge? currEdge
                        = (Edge)((ArrayList)adjList[currNode])[listIndex];
